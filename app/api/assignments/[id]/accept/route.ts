...
export async function POST(_:Request,{params}:{params:{id:string}}){
  ...
  const row=await prisma.assignment.update({ where:{ id: params.id }, data:{ status:"accepted" } })
  await prisma.request.update({ where:{ id: row.requestId }, data:{ status:"in_progress" } })
