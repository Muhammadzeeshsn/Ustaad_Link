...
export default function AdminClient() {
  ...
  // who am I? (used for header/signout context only; gating is done on server)
  const { data: meRes } = useSWR<Me | { data?: Me; user?: Me }>(
    '/api/me',
    swrFetcher,
    { shouldRetryOnError: false }
  )
  const me: Me | undefined =
    (meRes && 'data' in (meRes as any) ? (meRes as any).data : (meRes as Me)) as Me | undefined
  ...
  const requests: RequestRow[] =
    reqRes && 'data' in (reqRes as any) ? ((reqRes as any).data ?? []) : []
  ...
  const applications: ApplicationRow[] =
    appRes && 'data' in (appRes as any) ? ((appRes as any).data ?? []) : []
  ...
  // simple loading while data populates
  if (!meRes && (loadingReqs || loadingApps)) {
        ...
        <Tabs defaultValue="requests" className="space-y-6">
          ...
          <TabsContent value="requests" className="space-y-4">
            ...
            {loadingReqs ? (
              ...
              <div className="space-y-4">
                {requests.map((r: RequestRow) => (  // ← typed
                  <Card key={r.id}>
                    <CardHeader>
          ...
          <TabsContent value="proposals" className="space-y-4">
            ...
            {loadingApps ? (
              ...
              <div className="space-y-4">
                {applications.map((p: ApplicationRow) => (  // ← typed
                  <Card key={p.id}>
                    <CardHeader>
                      <div className="flex items-start justify-between">
                        <div>
