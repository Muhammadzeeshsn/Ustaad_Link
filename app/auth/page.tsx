...
const toUpperRole = (r: RoleTab): RoleUpper =>
  r === "student" ? "STUDENT" : "TUTOR";
const normalizeEmail = (e: string) => e.trim().toLowerCase();
const phoneOk = (v: string) => v && v.match(/^\+\d{1,3}\d{4,}$/);
...
export default function AuthPage() {
  ...
  // signup verify
  const [signupChallengeId, setSignupChallengeId] = useState<string | null>(
    null
  );
  const [signupOtp, setSignupOtp] = useState("");
  const [verifyingSignup, setVerifyingSignup] = useState(false);
  const [signupStep, setSignupStep] = useState<"form" | "otp" | "verified">(
    "form"
  );
...
const canSubmit = useMemo(() => {
  ...
  async function verifyOtp(params: any) {
    ...
    const j = await res.json().catch(() => ({}));
    if (!res.ok) {
      if (j?.error === "bad_code" || j?.error === "invalid_code")
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    try {
      ...
      if (!phone || !phoneOk(phone)) {
        setInlineError("Please enter a valid phone number with country code");
        return;
      }

      // needs to be reconsidered (temporary)
      setSubmitting(true);
    ...
    try {
      ...
      if (check === "ok" || check === "legacy") {
        const res = await signIn("credentials", {
          ...
          redirect: false,
          callbackUrl:
            roleUpper === "STUDENT" ? "/dashboard/student" : "/dashboard/tutor",
        });
        if (res?.ok) {
          return router.push(
            res.url ??
  ...
  async function sendResetOtp(e?: React.FormEvent) {
    ...
    try {
      ...
      if (status === 404) {
        setInlineError(
          "No account found with this email in the selected role."
        );
        return;
      }
      if (status === 429) {
        ...
        setTimerTick(0);
        setInlineNote(
          `Please wait ${data?.remain ?? 45}s before requesting another code.`
        );
        return;
      }
      if (status !== 200) {
  ...
  async function submitNewPassword(e?: React.FormEvent) {
    ...
    try {
      ...
      if (!challengeId)
        throw new Error("Missing verification. Please verify OTP again.");
      ...
      <section className="container mx-auto px-4 py-10 md:py-16">
        ...
        >
            ...
            <div className="relative h-full p-8 text-white">
              ...
              <div className="mx-auto max-w-sm">
                <h1 className="text-3xl font-extrabold">
                  Welcome to UstaadLink
                </h1>
                <p className="mt-3 text-white/90">
                  One simple account—for Students & Tutors. Admin-reviewed for
                  quality.
                </p>
                <ul className="mt-6 space-y-2 text-sm text-white/95">
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4" /> Admin-approved
                    matching
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4" /> Track request
                    status end-to-end
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle2 className="mt-0.5 h-4 w-4" /> Email
          ...
          <div className="bg-white p-6 md:p-8">
            <Tabs
              value={role}
              onValueChange={(v) => setRole(v as RoleTab)}
              className="w-full"
            >
                ...
                {loginOtpNeeded ? (
                  <div className="space-y-4">
                    ...
                    <div className="text-sm text-slate-600">
                      We sent a code to <b>{email}</b>. It expires in 2 minutes.
                    </div>
                    <SixBoxOtp value={loginOtp} onChange={setLoginOtp} />
                    <div className="flex items-center gap-2 pt-2">
                      <BrandButton
                        type="button"
                        onClick={verifyLoginOtpAndRetry}
                        disabled={loginOtp.replace(/\D/g, "").length !== 6}
                      >
                        Verify & continue
                      </BrandButton>
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (status === 429) {
                            ...
                            setTimerTick(0);
                            setInlineNote(
                              `Please wait ${remain}s before requesting another code.`
                            );
                          } else if (status === 200) {
                        ...
                        <>
                          ...
                          <div className="flex items-center justify-between pt-2">
                            <BrandButton
                              type="button"
                              disabled={
                                signupOtp.replace(/\D/g, "").length !== 6 ||
                                verifyingSignup ||
                                !signupChallengeId
                              }
                              onClick={confirmSignupOtp}
                            >
                            ...
                            <button
                              ...
                              className={`text-sm underline-offset-2 ${
                                remaining > 0
                                  ? "text-slate-400"
                                  : "text-[color:var(--brand,#0B1533)] hover:underline"
                              }`}
                            >
                              {remaining > 0
                                ? `Resend in ${remaining}s`
                        ...
                        <>
                          ...
                          <div className="flex items-center justify-between pt-2">
                            ...
                            <button
                              type="button"
                              onClick={openForgot}
                              className="text-sm text-[color:var(--brand,#0B1533)] underline-offset-2 hover:underline"
                            >
                              Forgot password?
                            </button>
                          </div>
                    ...
                    >
                      {forgotStage === "email" && (
                        <form onSubmit={sendResetOtp} className="space-y-4">
                          ...
                          <div className="flex items-center gap-2 pt-2">
                            <BrandButton
                              type="submit"
                              disabled={sendingReset || remaining > 0}
                            >
                      ...
                      {forgotStage === "otp" && (
                        <form onSubmit={verifyResetOtp} className="space-y-4">
                          ...
                          <div className="text-sm text-slate-600">
                            We sent a code to <b>{resetEmail}</b>. It expires in
                            2 minutes.
                          </div>
